...
import com.ms.dob.collage.Area;
...
import static com.ms.dob.collage.skew.SkewUtils.createLine;
import static com.ms.dob.collage.skew.SkewUtils.cutAreaCross;
import static com.ms.dob.collage.skew.SkewUtils.cutAreaWith;

/**
 * Oblique layout, the peripheral area is a rectangle
 *
 */
public abstract class SkewCollageLayout implements CollageLayout {
  private RectF bounds;
  private SkewArea outerArea;

  private List<Line> outerLines = new ArrayList<>(4);
  private List<SkewArea> areas = new ArrayList<>();
  ...
  private Comparator<SkewArea> areaComparator = new SkewArea.AreaComparator();

  private ArrayList<Step> steps = new ArrayList<>();

  protected SkewCollageLayout() {
  ...
  public void setOuterBounds(RectF bounds) {
    ...
    SkewLine lineLeft = new SkewLine(leftTop, leftBottom, Line.Direction.VERTICAL);
    SkewLine lineTop = new SkewLine(leftTop, rightTop, Line.Direction.HORIZONTAL);
    SkewLine lineRight = new SkewLine(rightTop, rightBottom, Line.Direction.VERTICAL);
    SkewLine lineBottom = new SkewLine(leftBottom, rightBottom, Line.Direction.HORIZONTAL);
  ...
  @Override
  public SkewArea getArea(int position) {
    sortAreas();
  ...
  protected List<SkewArea> addLine(int position, Line.Direction direction, float ratio) {
    return addLine(position, direction, ratio, ratio);
  }

  protected List<SkewArea> addLine(int position, Line.Direction direction, float startRatio,
                                   float endRatio) {
    SkewArea area = areas.get(position);
    areas.remove(area);
    SkewLine line = createLine(area, direction, startRatio, endRatio);
    lines.add(line);

    List<SkewArea> increasedAreas = cutAreaWith(area, line);
  ...
  protected void addCross(int position, float startRatio1, float endRatio1,
    ...
    SkewLine horizontal = createLine(area, Line.Direction.HORIZONTAL, startRatio1, endRatio1);
    SkewLine vertical = createLine(area, Line.Direction.VERTICAL, startRatio2, endRatio2);
    lines.add(horizontal);
    lines.add(vertical);

    List<SkewArea> increasedAreas = cutAreaCross(area, horizontal, vertical);
  ...
  protected void cutArea(int position, int hSize, int vSize) {
    ...
    Pair<List<SkewLine>, List<SkewArea>> spilt =
        SkewUtils.cutAreaWith(area, hSize, vSize);
